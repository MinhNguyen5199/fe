...
export default function BookDetailPage({ params: paramsPromise }: BookDetailPageProps) {
  ...
  if (error) {
    return <div className="container mx-auto p-4 text-center text-red-600 bg-red-100 border border-red-300 rounded-md my-8 max-w-xl">Error: {error}</div>;
  }
        ...
        <main className="pt-4">
            <div className="p-6 max-w-4xl mx-auto my-8 bg-white dark:bg-gray-900 rounded-lg shadow-xl mt-11">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div>
                  <p className="mb-2 text-gray-700 dark:text-gray-300"><strong className="font-semibold text-gray-800 dark:text-gray-200">Author(s):</strong> {book.authors.join(', ')}</p>
                  <p className="mb-2 text-gray-700 dark:text-gray-300"><strong className="font-semibold text-gray-800 dark:text-gray-200">Genre(s):</strong> {book.genres.join(', ')}</p>
                  {book.publication_date && (
                    <p className="mb-2 text-gray-700 dark:text-gray-300">
                      <strong className="font-semibold text-gray-800 dark:text-gray-200">Publication Date:</strong> {new Date(book.publication_date).toLocaleDateString()}
                    </p>
                  )}
                </div>
              ...
              {book.description && (
                <div className="mb-8">
                  <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">Description</h2>
              ...
              <div className="mb-8">
                ...
                {mainSummary && !showSummaryEditor ? (
                  <>
                    <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-5 mb-4 bg-gray-50 dark:bg-gray-800/50 shadow-sm">
                      <div
                  ...
                  userProfile?.is_admin && showSummaryEditor ? (
                    <form onSubmit={handleSaveSummary} className="p-6 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50 shadow-sm">
                      ...
                      {summaryMessage && <p className="mt-4 p-3 bg-green-100 text-green-700 border border-green-300 rounded-md text-center">{summaryMessage}</p>}
                      {summaryError && <p className="mt-4 p-3 bg-red-100 text-red-700 border border-red-300 rounded-md text-center">{summaryError}</p>}
                    </form>
                  ) : (
              ...
              <div className="mb-8">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">Affiliate Links</h2>
                  ...
                  <ul className="list-disc pl-6 space-y-2">
                    {book.affiliate_links.map((link: AffiliateLink, index: number) => (
                      <li key={index} className="text-gray-700 dark:text-gray-300">
                        ...
                        <a href={link.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline break-words">{link.url}</a>
                      </li>
                    ))}
                  </ul>
                ) : (
                ...
                <>
                  {(userProfile.current_tier?.toLowerCase().includes('vip')) ? (
                    ...
                    <div className="my-8 rounded-2xl bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-200 p-6 text-center shadow-md">
                      ...
                      <p>Upgrade to <span className="font-bold">VIP</span> to access personalized video summaries.</p>
                    </div>
                  )}
                  ...
                  {['vip', 'pro'].some(tier => userProfile.current_tier?.toLowerCase().includes(tier)) ? (
                    ...
                    <div className="my-8 rounded-2xl bg-purple-100 dark:bg-purple-900/30 text-purple-900 dark:text-purple-200 p-6 text-center shadow-md">
                      ...
                      <p>Upgrade to <span className="font-bold">VIP</span> or <span className="font-bold">PRO</span> to listen on the go.</p>
                    </div>
                  )}
                </>
              )}
