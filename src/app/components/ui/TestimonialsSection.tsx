            ...
            >
              <div className="flex justify-center mb-4">
                {Array.from({ length: 5 }, (_, i) => (
                  <Star
                    key={i}
                    className={`w-5 h-5 ${i < testimonial.rating ? 'text-amber-500 fill-current' : 'text-gray-300 dark:text-gray-600'}`}
                  />
                ))}
              </div>
              ...
              <div className="flex items-center justify-center space-x-4">
                <img
                  ...
                  className="w-16 h-16 rounded-full object-cover object-center border-4 border-indigo-200 dark:border-indigo-700 shadow-md"
                  onError={(e) => {
                    const target = e.target as HTMLImageElement;
                    target.onerror = null;
                    target.src = `https://placehold.co/80x80/666/EEE?text=Avatar`;
                  }}
                />
                <div>
